""" Windows Packager script for Bookmarks.

The script build the Bookmarks.exe binary and collects the lkibrary dependencies
needed to run the script.

Modify the *_ROOT variables to point to the respective dependencie's install
directory.

NOTE: There's no automated build script for the dependencies at the moment, so
make sure you're linking against the same includes and libraries as used for
building ALembic and OpenImageIO.
...
DUMPBIN = ur'"' + VC_ROOT + os.path.sep + ur'dumpbin.exe"'
...
PACKAGE_ROOT = os.path.dirname(__file__) + ur'\..\..\..\bookmarks-standalone'
...
def get_dependencies():
    ...
    def get_lib_dependencies(libpath, l):
        ...
        _dep = [f for f in _dep if not [
            s for s in builtins if s.lower() in f.lower()]]
        l = sorted(list(set(l)))
        if not _dep:
            return l
        l += _dep
        for libpath in _dep:
            l = get_lib_dependencies(libpath, l)
        l = sorted(list(set(l)))
        return l
    ...
    for L in TOPLIBS:
        ...
        dependencies = get_lib_dependencies(L, dependencies)
...
def make_folders():
    ...
    if not os.path.isdir(root + os.path.sep + u'lib' + os.path.sep + 'site-packages' + os.path.sep + 'PySide2'):
        os.mkdir(root + os.path.sep + u'lib' + os.path.sep + 'site-packages' + os.path.sep + 'PySide2')
    if not os.path.isdir(root + os.path.sep + u'lib' + os.path.sep + 'site-packages' + os.path.sep + 'shiboken2'):
        os.mkdir(root + os.path.sep + u'lib' + os.path.sep + 'site-packages' + os.path.sep + 'shiboken2')
...
def copy_libs(libs):
    ...
    if not os.path.isfile(pzip):
        raise RuntimeError('Python module zip is missing')
    ...
    copy_tree(
        ...
        root + os.path.sep + u'lib' + os.path.sep + 'site-packages' + os.path.sep + 'shiboken2' + os.path.sep + 'files.dir'
    )

    shutil.copy2(
        QT5_ROOT + os.path.sep + 'plugins' + os.path.sep + 'platforms' + os.path.sep + 'qwindows.dll',
...
def build_bin():
    ...
    shutil.copy2(
        ...
        PACKAGE_ROOT + os.path.sep + u'bookmarks' + os.path.sep + 'bookmarks.exe')
    shutil.copy2(
        bindir + os.path.sep + 'bookmarks_d.exe',
        PACKAGE_ROOT + os.path.sep + u'bookmarks' + os.path.sep + 'bookmarks_d.exe')
    shutil.move(
...
def install_python_modules():
    ...
    target = PACKAGE_ROOT + os.path.sep + u'bookmarks' + os.path.sep + 'shared'
