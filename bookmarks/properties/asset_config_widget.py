...
class FormatEditor(QtWidgets.QDialog):
    ...
    def _create_ui(self):
        ...
        self.listwidget = list_widget.ListWidget(parent=self)
        ...
        self.save_button = common_ui.PaintedButton('Save')
        self.cancel_button = common_ui.PaintedButton('Cancel')
...
class SubfolderEditor(QtWidgets.QDialog):
    ...
    def _create_ui(self):
        ...
        for _k, _v in sorted(self.v['subfolders'].items(), key=lambda x: x[1]['name']):
            ...
            _row = common_ui.add_row(_v['name'], parent=grp)
            editor = common_ui.LineEdit(parent=_row)
        ...
        for section, section_name in SECTIONS:
            ...
            for k, v in sorted(data[section].items(), key=lambda x: x[1]['name']):
                ...
                if section == asset_config.FileNameConfig:
                    ...
                    button = common_ui.PaintedButton(u'+', parent=row)
                ...
                button = common_ui.PaintedButton(u'Formats', parent=row)
                row.layout().addWidget(button, 0)
                if 'filter' in v:
                ...
                button = common_ui.PaintedButton(u'Subfolders', parent=row)
                row.layout().addWidget(button, 0)
