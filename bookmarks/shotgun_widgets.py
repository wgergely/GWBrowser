...
class CreateTaskPublish(QtWidgets.QDialog):
    ...
    def _connect_signals(self):
        ...
        self.user_picker.currentTextChanged.connect(
            lambda s: settings.local_settings.setValue(u'shotgun/user', s))
        self.storage_picker.currentTextChanged.connect(
            lambda s: settings.local_settings.setValue(u'shotgun/storage', s))
        self.type_picker.currentTextChanged.connect(
            lambda s: settings.local_settings.setValue(u'shotgun/type', s))
    ...
    def init_values(self):
        try:
            ...
            if storage:
                ...
                try:
                    storage = sorted(storage, key=lambda x: x['id'])
                except:
                    pass
                ...
                for store in storage:
                    ...
                    if 'code' in store:
                        ...
                        name = 'LocalStorage{}'.format(store['id'])
                    else:
                        continue
                    ...
                    self.storage_picker.addItem(name, userData=store['id'])

                prev = settings.local_settings.value(u'shotgun/storage')
                if prev:
            ...
            if types:
                ...
                for type in types:
                    ...
                    if 'code' in type:
                        ...
                        name = 'PublishedFileType{}'.format(type['id'])
                    else:
                        continue
                    ...
                    self.type_picker.addItem(name, userData=store['id'])

                prev = settings.local_settings.value(u'shotgun/type')
                if prev:
    ...
    def action(self):
        try:
            ...
            with shotgun.init_sg(domain, script_name, api_key):
                ...
                url = u'{}/detail/PublishedFile/{}'.format(domain, entity['id'])
                QtGui.QDesktopServices.openUrl(url)
