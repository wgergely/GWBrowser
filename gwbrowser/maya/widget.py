...
command:
    ...
    import gwbrowser.maya.widget as mGWBrowser mGWBrowser.show()
...
def export_alembic(destination_path, outliner_set, startframe, endframe, step=1.0):
    ...
    try:
        ...
        perframecallback = u'"import gwbrowser.maya.widget as w;w.report_export_progress({}, #FRAME#, {}, {})"'.format(
...
def capture_viewport(size=1.0):
    ...
    """
    import gwbrowser.maya._mCapture as mCapture
    ext = u'png'
...
class MayaBrowserWidget(MayaQWidgetDockableMixin, QtWidgets.QWidget):
    ...
    def save_warning(self, *args):
        ...
        if workspace_info.path().lower() not in scene_file.filePath().lower():
            ...
            mbox.setInformativeText(
                u'If you didn\'t expect this message, is it possible the project was changed by GWBrowser from another instance of Maya?')
            mbox.setStandardButtons(QtWidgets.QMessageBox.Ok)
            mbox.setDefaultButton(QtWidgets.QMessageBox.Ok)
            res = mbox.exec_()
